				...
				>
					<div style={{ fontSize: 11, lineHeight: 1.5 }}>
						<div style={{ fontSize: 14, fontWeight: 'bold', marginBottom: 5 }}>
							{formatValue(fatturaData.emittente.nome)}
						</div>
						...
						{fatturaData.emittente.pec && (
							<div>PEC: {fatturaData.emittente.pec}</div>
						)}
						{fatturaData.emittente.telefono && (
							<div>Tel: {fatturaData.emittente.telefono}</div>
						)}
					</div>
					<div style={{ textAlign: 'right' }}>
						<h3 style={{ fontSize: 18, fontWeight: 'bold', margin: 0 }}>
							n. {formatValue(fatturaData.numero)}
						</h3>
						<p style={{ margin: 0, fontSize: 12, color: '#666' }}>
							del {formatValue(fatturaData.data)}
						</p>
					</div>
				</div>
			</header>
			...
			>
				...
				>
					...
					<div style={{ fontSize: 13, fontWeight: 'bold' }}>
						{formatValue(fatturaData.cliente.nome)}
					</div>
					<div style={{ fontSize: 11, color: '#4b5563', marginTop: 3 }}>
						{formatValue(fatturaData.cliente.indirizzo)}
						<br />
						P.IVA: {formatValue(fatturaData.cliente.partita_iva)}
						<br />
				...
				>
					...
					>
						...
						<span style={{ fontWeight: 'semibold' }}>
							€{fatturaData.totale_imponibile.toFixed(2)}
						</span>
					</div>
					<div
						style={{
					...
					>
						...
						<span style={{ fontWeight: 'semibold', color: '#dc2626' }}>
					...
					>
						<span style={{ color: '#6b7280' }}>
							IVA ({fatturaData.iva_percentuale}%):
						</span>
						<span style={{ fontWeight: 'semibold' }}>
							€
							{(
								fatturaData.totale_scontato *
								(fatturaData.iva_percentuale / 100)
							).toFixed(2)}
						</span>
					</div>
					<div
