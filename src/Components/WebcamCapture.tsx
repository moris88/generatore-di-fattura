	...
	useEffect(() => {
		async function startCamera() {
			try {
				...
				setError(
					'Impossibile accedere alla webcam. Assicurati di aver dato i permessi.',
				)
				setLoading(false)
			}
		}
		...
		<div className="fixed inset-0 z-100 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
			<div className="relative flex w-full max-w-2xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
				<div className="flex items-center justify-between border-b p-4">
					<h3 className="font-bold text-gray-800 text-lg">Scatta una foto</h3>
					<button
						type="button"
				...
				<div className="relative aspect-video w-full bg-black">
					...
					{error && (
						<div className="flex h-full flex-col items-center justify-center p-6 text-center text-red-500">
							...
							<button
								type="button"
								className="mt-4 rounded-lg bg-red-100 px-4 py-2 font-semibold text-red-700 text-sm hover:bg-red-200"
								onClick={onClose}
							>
								Chiudi
							</button>
				...
				<div className="bg-gray-50 p-3 text-center text-gray-500 text-xs">
