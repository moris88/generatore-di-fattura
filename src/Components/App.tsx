			...
			<div className="flex w-full max-w-5xl flex-col items-center justify-center gap-8 lg:p-6">
				<div className="w-full rounded-2xl bg-white p-4 shadow-md sm:p-6">
					<div className="mb-6 flex flex-col items-center justify-between gap-4 border-b pb-6 sm:flex-row">
						<h1 className="font-black text-blue-700 text-xl uppercase tracking-tight sm:text-2xl">
							Generatore di fattura
						</h1>
						<button
							type="button"
							onClick={() => setShowResetPopup(true)}
							className="flex items-center gap-2 rounded-lg bg-red-50 px-4 py-2 font-bold text-red-600 text-xs transition-colors hover:bg-red-100 active:scale-95"
						>
					...
					<div className="mt-8 grid grid-cols-1 gap-6 md:grid-cols-2">
						<div className="flex flex-col gap-2">
							<label
								htmlFor="technical-signature"
								className="font-bold text-gray-700 text-sm uppercase"
							>
								Firma digitale responsabile:
							</label>
							<div className="overflow-hidden rounded-lg border-2 border-gray-300 border-dashed bg-gray-50">
								<SignPad
						...
						<div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
							<div className="flex flex-col gap-2">
								...
								<input
									...
									className="block w-full cursor-pointer text-gray-500 text-sm file:mr-4 file:rounded-full file:border-0 file:bg-blue-50 file:px-4 file:py-2 file:font-semibold file:text-blue-700 file:text-sm hover:file:bg-blue-100"
									type="file"
							...
							<div className="flex flex-col gap-2">
								...
								<div className="flex flex-col gap-2">
									<input
										...
										multiple
										accept="image/*"
										className="block w-full flex-1 cursor-pointer text-gray-500 text-sm file:mr-4 file:rounded-full file:border-0 file:bg-green-50 file:px-4 file:py-2 file:font-semibold file:text-green-700 file:text-sm hover:file:bg-green-100"
										type="file"
										onChange={handleAddPhoto}
									/>
									<button
										type="button"
										className="flex items-center justify-center gap-2 rounded-lg bg-blue-50 px-4 py-2 font-semibold text-blue-700 text-sm transition-colors hover:bg-blue-100"
						...
						{attachments.length > 0 && (
							<div className="mt-4 overflow-hidden rounded-xl border bg-gray-50">
								<button
									...
									className="flex w-full items-center justify-between bg-gray-100 px-4 py-2 font-bold text-gray-700 text-sm"
								>
										...
										{attachments.map((file, idx) => (
											...
											>
												<div className="mr-2 flex flex-col overflow-hidden">
													<span className="truncate font-bold text-gray-800 text-xs">
														{file.name}
													</span>
												...
												<div className="flex items-center gap-1">
													<button
														...
														className="rounded-md bg-blue-50 px-2 py-1 font-bold text-[10px] text-blue-600 hover:bg-blue-100"
													>
														VEDI
													</button>
													<button
					...
					<div className="mt-8 flex items-center justify-center gap-4">
						<button
							type="button"
							className={`btn btn-primary flex w-full items-center justify-center gap-2 py-4 transition-all duration-300 sm:w-auto sm:px-12 ${!isFormValid ? 'cursor-not-allowed opacity-50 grayscale' : 'hover:scale-105'}`}
				...
				{generated && showPreview && (
					<div
						...
						id="preview-section"
					>
						<div className="flex items-center gap-2 rounded-full bg-blue-100 px-4 py-1 font-bold text-blue-700 text-xs">
						...
						{attachmentPreviews.length > 0 && (
							<div className="w-full space-y-8">
								<div className="mx-auto flex w-fit items-center gap-2 rounded-full bg-green-100 px-4 py-1 font-bold text-green-700 text-xs">
									ANTEPRIMA ALLEGATI
								</div>
								{attachmentPreviews.map((preview, idx) => (
									...
									>
										<div className="flex justify-between border-b bg-gray-50 px-4 py-2 font-bold text-[10px] text-gray-500 uppercase">
											<span>
											...
											{preview.type.startsWith('image/') ? (
												...
												<div className="flex flex-col items-center gap-4 py-12">
													...
													<button
														...
														className="font-bold text-blue-600 text-xs underline"
													>
					...
					<div className="relative flex h-full w-full max-w-5xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
						<div className="flex items-center justify-between border-b p-4">
							<div className="flex flex-col">
								<h3 className="max-w-xs truncate font-bold text-gray-800 text-sm sm:max-w-md">
									{previewFile.name}
								</h3>
								<span className="text-[10px] text-gray-500 uppercase">
									{previewFile.type}
